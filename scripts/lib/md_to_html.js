// scripts/lib/md_to_html.js — shared Markdown → styled HTML for deliverables.
//
// The /constitution skill writes Markdown; this turns it into a self-contained,
// print-ready HTML page, then render_pdf.py converts it to the shareable PDF.
// One visual language across every deliverable.
//
// Deliberately dependency-free: a small, predictable Markdown subset (headings,
// bold/italic/code, links, lists, tables, hr, paragraphs), with HTML escaped so
// data can't break the layout.
...
.report-header { background: linear-gradient(135deg,#5e5ce6,#7d2ae8); color: #fff; border-radius: 20px;
...
export function mdToHtml(md, { title = "pbOS Brand-of-One", subtitle = "" } = {}) {
  return `<!DOCTYPE html>
...
${mdToFragment(md)}
<div class="report-footer">Generated by pbOS · ${esc(new Date().toISOString().slice(0, 10))}</div>
</div></body></html>`;
}

/**
 * Write HTML next to a markdown doc and render a PDF via render_pdf.py.
